...
module.exports = {
	...
	generate: function(fileName, data, missing, extra) {
		...
		newFileName = fileRegExp.exec(fileName)[0],
		prefix = '$EN-' + newFileName.replace(/\.[a-zA-Z]+/, '').toUpperCase(),
		formatterJson,
		arrayValue;
		...
		for (var i = 0; i < missing.length; i++) {
			...
			// if ture we need wrap every item with ${} in array
			if (isArray(value)) {
				arrayValue = Array.prototype.map.call(value, function(item) {
					return prefix + '{' + item + '}';
				}) || [];
		...
		formatterJson = JSON.stringify(data, null, 2);
